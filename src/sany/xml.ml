...
let source_to_sany_xml_str (module_path : string) (stdlib_path : string) : (string, (string option * string)) result =
  ...
  *)
  let cmd = Printf.sprintf "java -cp %s%s tla2sany.xml.XMLExporter -t -I %s -I %s %s %s"
    (backend_classpath_string "tla2tools.jar")
    (match !Params.module_jar_paths with | [] -> "" | paths -> ":" ^ (String.concat ":" paths))
    (Filename.dirname module_path)
    (Filename.quote stdlib_path)
    (match !Params.rev_search_path with | [] -> "" | paths -> "-I " ^ (String.concat " -I " paths))
    (Filename.quote module_path)
  in if Params.debugging "sany" then print_endline cmd else ();
...
let xml_to_leibniz_param (xml : tree) : leibniz_param =
  match xml with
...
and xml_to_user_defined_op_kind (children : tree list) : user_defined_op_kind =
  ...
  | node,
    Node ("uniquename", [SValue name])
    :: Node ("arity", [IValue arity])
    :: Node ("originalOperator", [Node ("UserDefinedOpKindRef", [Node ("UID", [IValue originalOp])])])
    :: Node ("originallyDefinedInModule", [Node ("ModuleNodeRef", [Node ("UID", [IValue originalModule])])])
    :: Node ("body", [body])
    :: Node ("params", parameters)
    :: children -> (
    let precomments, flags = match children with
    | Node ("pre-comments", [SValue precomments]) :: flags -> Some precomments, flags
    | _ -> None, children
    in {
      node;
      name;
...
let xml_to_theorem_node (children : tree list) : theorem_node =
  ...
  | node, definition, Node ("body", [body]) :: proof -> {
      ...
      suffices    = List.exists (is_tag "suffices") proof;
    }
  | _ -> ls_conversion_failure __FUNCTION__ children
