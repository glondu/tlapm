(** This module converts SANY's abstract syntax tree (AST) into TLAPM's AST.
    ...
    because we have to attach the name (sans level) as metadata properties on
    each proof step. We need to parse references to named instanced modules
    because if a module is imported with M == INSTANCE Modname, all
...
(** Utility function constructing & raising an exception for when conversion
    ...
    planned although could be added in the future. It is possible that calls
    to this function actually occupy gaps in the mapping from SANY's XML
    format to TLAPM's internal AST which are not actually possible to
    instantiate with a valid TLA+ spec. However, if a user encounters this
    failure mode with a real TLA+ spec then it should be considered a bug
    or shortfall with this conversion process.
*)
...
(** Converts operator declarations such as CONSTANTS and VARIABLES. In a
    ...
    single Variables modunit. Returns None for declarations inlined into this
    module by EXTENDS or INSTANCE.
*)
and convert_op_decl_node (xml : Xml.op_decl_node) (enclosing_module_name : string) : Module.T.modunit option =
  (* Skip inlined declarations. TODO: modify SANY to add originalModule field to OpDeclNode *)
  if (Option.get xml.node.location).filename <> enclosing_module_name then None else
  match xml.kind with
  | Constant -> Some (attach_props xml.node (Constants [attach_props xml.node xml.name, match xml.arity with | 0 -> Shape_expr | n -> Shape_op n]))
  | Variable -> Some (attach_props xml.node (Variables [attach_props xml.node xml.name]))
...
and convert_non_tuply_bounds (node : Xml.node) (bound : Xml.bound_symbol) : bounds =
  ...
  | hd :: tl -> (hd, Constant, Domain (convert_expression bound.expression))
    :: List.map (fun s -> (s, Constant, Ditto)) tl
...
(** Converts subexpressions and labels. SANY does not export sufficient info
    to convert basic cases like op!1 (TODO: fix SANY XML Exporter), so only
    some cases are handled here.
*)
and convert_subexpression (apply : Xml.op_appl_node) : Expr.T.expr =
  match apply.operands with
  | [Expression (LabelNode {body = Expression expr; _})] -> convert_expression expr
  | [Expression (LabelNode {body = AssumeProveLike _ as body; node; _})] ->
    Sequent (convert_sequent body) |> attach_props node
  | [operand] -> convert_expression_or_operator_argument apply.node operand
  | _ -> conversion_failure "Expected a single operand to subexpression ($Nop) operator" apply.node.location
...
and convert_definition_reference (node : Xml.node) (name : string) (op_or_apply : [ `Op | `Apply of Xml.expr_or_op_arg list]) : Expr.T.expr =
  ...
  | [component] -> (
    (* SANY and TLAPM can use different canonical names for the same operator,
       like SANY's \geq vs. TLAPM's >=. Normalize using this Optable lookup. *)
    let name = (Optable.lookup component).name in
...
(** Converts theorem nodes. Oddly, SANY has two different theorem node types
    ...
    unchanged for error message purposes. Returns None for theorems inlined
    into this module by EXTENDS or INSTANCE.
*)
and convert_theorem_node (uid : int) (previous_proof_level : int) (thm : Xml.theorem_node) (enclosing_module_name : string) : Module.T.modunit option =
  (* Skip theorems inlined by EXTENDS or INSTANCE; TODO add originalModule field on SANY side *)
  if (Option.get thm.node.location).filename <> enclosing_module_name then None else
  ...
  Theorem (
    ...
    proof,
    empty_summary
  ) |> attach_props thm.node |> Option.some
...
and convert_assume_prove (ap : Xml.assume_prove_node) : sequent =
  let convert_hypothesis (hypothesis : Xml.assumption_kind) : Expr.T.hyp =
    ...
    | NewSymbol ns ->
      ...
      | Some domain -> Bounded (convert_expression domain, Visible)
      | None -> Unbounded
...
and convert_proof_step_name (node : Xml.node) (uid : int) (proof_level : int) (theorem_def_ref : int option) : stepno =
  ...
  | Some uid ->
    ...
    let name_start = (String.index proof_name '>') + 1 in
...
and convert_proof_step (node : Xml.node) (proof_level : int) (step : Xml.proof_step_group) : Proof.T.step =
  ...
  | DefStep {node; def_refs} ->
    convert_definition_proof_step node def_refs |> attach_proof_step_name (Unnamed (proof_level, 0))
  | UseOrHide use_or_hide ->
    (if use_or_hide.hide
     then Hide (convert_usable use_or_hide)
     else Use (convert_usable use_or_hide, use_or_hide.only))
    |> attach_props use_or_hide.node |> attach_proof_step_name (Unnamed (proof_level, 0))
  | TheoremNodeRef uid ->
    let thm = resolve_theorem_node node uid in
    ...
    (* SUFFICES ASSUME ... PROVE ... is encoded as an assume/prove body with
       a theorem-level "suffices" flag rather than a $Suffices operator. *)
    | _ when thm.suffices -> Suffices (convert_sequent thm.body, proof)
    | _ -> Assert (convert_sequent thm.body, proof)
...
let convert_ast (ast : Xml.modules) : (Module.T.modctx * Module.T.mule, (string option * string)) result =
  entries :=
    List.fold_left
      ...
      Coll.Im.empty
      ast.context;
  let mcx : Module.T.modctx = List.fold_left
    (fun (map : Module.T.modctx) (mule_ref : int) ->
      ...
      | None when mule.name = "TLAPS" ->
        (* The TLAPS standard module defines backend pragmas like PTL and SMT
           in special (*{ by (...) }*) comments that SANY does not export. Thus,
           load the module using TLAPM's own parser for now.
           TODO: Define TLAPS as a standard module in Module.Standard.initctx
        *)
        if Params.debugging "sany" then print_endline "Parsing module TLAPS with TLAPM parser" else ();
        Coll.Sm.add mule.name (Module.Save.parse_file (noprops (mule.name ^ ".tla"))) map
      | None ->
    ...
    Coll.Sm.empty
    ast.module_refs
  in let root_module = Coll.Sm.find ast.root_module mcx in
  root_module.core.important <- true;
  if Params.debugging "sany" then print_endline ("Root module " ^ root_module.core.name.core) else ();
  Ok (mcx, root_module)
